...
namespace RobotParty
{
        ...
        public void Create(int option) {
            switch (option) {
                ...
                case 0:
                    ...
                    */
                    elements.Add(new CircleEnemyCharacter(new Tuple<float, float>(300, 300), 50, 10, this, 100));
                    elements.Add(new CircleEnemyCharacter(new Tuple<float, float>(310, 310), 50, 15, this, 100));
                    elements.Add(new CircleEnemyCharacter(new Tuple<float, float>(310, 310), 50, 50, this, 100));
                    elements.Add(new CircleEnemyCharacter(new Tuple<float, float>(330, 330), 50, 10, this, 100));
    ...
    public class CircleEnemyCharacter : EnemyCharacter
    {
        ...
        private Tuple<float, float> GetTarget() {
            ...
            var x_1 = (int)Math.Round(radius * Math.Cos(Math.PI / 180 * step) + x_0);
            var y_1 = (int)Math.Round(radius * Math.Sin(Math.PI / 180 * step) + y_0);

            target = new Tuple<float, float>(x_1, y_1);
            if (step == 360) step = 0;
            step++;
            return target;
        }
